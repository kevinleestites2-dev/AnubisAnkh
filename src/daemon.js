...
class Daemon {
  ...
  async _cycle(cfg) {
    try {
      ...
      if (!thought) return; // nothing worth saying
      ...
      memory.store(`daemon reached out: "${thought.slice(0, 80)}..."`, ['daemon', 'proactive']);
    } catch (err) {
  ...
  async _reflect(cfg) {
    ...
    if (timeSinceChat < 15 * 60 * 1000) return null; // within 15 min — stay quiet
